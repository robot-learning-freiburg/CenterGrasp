...
def extract_latent_emb_from_peaks(heatmap_output, peaks, shape_emb_output, appearance_emb_output, scale_factor=8):
  ...
  return shape_embeddings, appearance_embeddings, indices, scores
...
def extract_abs_pose_from_peaks(peaks, abs_pose_output, scale_factor=8):
  ...
  for ii in range(peaks.shape[0]):
    ...
    abs_poses.append(transform.Pose(
            camera_T_object=transformation_mat
        ))
  return abs_poses
