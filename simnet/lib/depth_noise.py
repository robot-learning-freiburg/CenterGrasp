...
class DepthManager():
    ...
    def distort(self, d_img):
        ...
        if self.depth_config_['border_distortion']:
            ...
            if num_nonzero == 0:
               return d_img
            dropout_centers_idx = np.random.choice(
            ...
            dropout_centers = high_gradient_px[dropout_centers_idx, :]
            self.apply_ellipses(d_img, dropout_centers, x_radii, y_radii)     
        ...
        if self.depth_config_['image_dropout']:
            ...
            dropout_centers = nonzero_px[dropout_centers_idx, :]
            self.apply_ellipses(d_img, dropout_centers, x_radii, y_radii)
    ...
    def apply_ellipses(self, d_img, dropout_centers, x_radii, y_radii):
        width, height = d_img.shape
        x_radii = np.round(x_radii).astype(int)
        y_radii = np.round(y_radii).astype(int)        
        # set interior pixels to zero
        for j in range(len(dropout_centers)):
            left_x = max(dropout_centers[j, 0] - x_radii[j], 0)
            right_x = min(dropout_centers[j, 0] + x_radii[j]+1, width)
            left_y = max(dropout_centers[j, 1] - y_radii[j], 0)
            right_y = min(dropout_centers[j, 1] + y_radii[j]+1, height)
            d_img[left_x:right_x, left_y:right_y] = 0.0
        return
