...
class Grid3D:
    def __init__(self, density=30, grid_dim: float = 1.0, device="cpu", precision=torch.float32):
        self.points = Variable(
            self.generate_point_grid(density, grid_dim).to(device, precision), requires_grad=True
        )
    ...
    def generate_point_grid(self, grid_density, grid_dim):
        ...
        grid_np = np.concatenate((X[..., None], Y[..., None], Z[..., None]), axis=-1).reshape(
            (-1, 3)
        )
    ...
    #     return (
    #         points_masked.to(pred_sdf_grid.to(pred_sdf_grid.dtype)),
