...
def cli():
    ...
    cc_texture_dir = Directories.TEXTURES
    cc_texture_dir.mkdir(parents=True)
    ...
    while True:
        ...
        json_url = (
            f"https://ambientcg.com/api/v2/full_json?include=downloadData&limit={offset_size}"
        ...
        if "foundAssets" in json_data and len(json_data["foundAssets"]) > 0:
            for asset in json_data["foundAssets"]:
                if (
                    "downloadFolders" in asset
                    and "default" in asset["downloadFolders"]
                    and "downloadFiletypeCategories" in asset["downloadFolders"]["default"]
                ):
                    current_download_dict = asset["downloadFolders"]["default"][
                        "downloadFiletypeCategories"
                    ]
                    if (
                        "zip" in current_download_dict
                        and "downloads" in current_download_dict["zip"]
                    ):
                        for download_attr in current_download_dict["zip"]["downloads"]:
                            if (
                                "attribute" in download_attr
                                and download_attr["attribute"] == "2K-JPG"
                            ):
                                data[asset["assetId"]] = (
                                    download_attr["fullDownloadPath"],
                                    download_attr["zipContent"],
                                )
                    else:
                        print(f"No zip or downloads found for asset: {asset['assetId']}")
                else:
                    print(
                        f"No downloadFolders or default or downloadFiletypeCategories found for asset: "
                        f"{asset['assetId']}"
                    )
        else:
            break
    ...
    for index, (asset, content) in enumerate(data.items()):
        ...
        if download_assets:
            ...
            try:
                ...
                extract_file(current_folder, file)
            except:
                print(f"Could not download asset: {asset}")
                continue
